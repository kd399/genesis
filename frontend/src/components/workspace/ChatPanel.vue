    ...
    <div ref="chatEl" class="flex-1 overflow-y-auto p-3 space-y-4">
      ...
      >
        ...
        <div v-if="msg.role === 'user'" class="max-w-[88%] space-y-1">
          <div
            class="bg-primary text-primary-foreground rounded-2xl rounded-tr-sm px-3 py-2 text-sm leading-relaxed"
          >
            {{ msg.content }}
          </div>
        ...
        <div v-else class="max-w-[92%] space-y-1.5">
          ...
          <div class="flex items-center gap-1.5 pl-0.5">
            <div
              class="w-5 h-5 rounded-full bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center shrink-0"
            >
          ...
          >
            ...
            >
              ...
              <span
                v-if="
                  i === msg.activities.length - 1 &&
                  isGenerating &&
                  msg.id === ws.messages[ws.messages.length - 1]?.id
                "
              ...
              <span class="leading-relaxed break-all">
                {{ activity.label }}
                <span v-if="activity.path" class="ml-1 font-mono text-[10px] opacity-70">{{
                  activity.path
                }}</span>
              </span>
            </div>
      ...
      <div v-if="ws.generationState.error" class="flex justify-start">
        <div
          class="bg-destructive/10 text-destructive rounded-xl px-3 py-2 text-xs max-w-[88%] flex items-start gap-2"
        >
